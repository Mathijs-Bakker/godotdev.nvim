...
return {
  ...
  {
    name = "health skips indentation scan outside a Godot project",
    run = function()
      local recorder = make_health_recorder()
      ...
      local joined_info = table.concat(recorder.calls.info, "\n")
      local joined_warns = table.concat(recorder.calls.warn, "\n")
      h.assert_truthy(joined_info:match("Indentation check skipped %(not in a Godot project%)") ~= nil)
      h.assert_falsy(joined_warns:match("Mixed indentation detected"))
    end,
  },
  {
    name = "health reports mixed indentation only when Godot project scan finds tabs",
    run = function()
      local recorder = make_health_recorder()
            ...
            h.with_package("dapui", {}, function()
              ...
              h.with_field(vim.fn, "exists", function(cmd)
                ...
                h.with_field(vim.fn, "executable", function(name)
                  ...
                  h.with_field(vim.fs, "find", function()
                    return { "/tmp/godot-project/project.godot" }
                  end, function()
                    h.with_field(vim, "system", function(argv, _opts)
                      return {
                        wait = function()
                          ...
                          if argv[1] == "rg" then
                            h.assert_equal(argv[#argv], "/tmp/godot-project")
                            return { code = 0, stdout = "/tmp/godot-project/player.gd:3:\tvar speed = 1\n", stderr = "" }
                          end
                          return { code = 0, stdout = "", stderr = "" }
                        end,
                      }
      ...
      local joined_warns = table.concat(recorder.calls.warn, "\n")
      h.assert_truthy(joined_warns:match("Mixed indentation detected") ~= nil)
    end,
  },
}
